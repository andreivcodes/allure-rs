...
fn main() {
    ...
    run_sample_test(&writer, "test_user_registration", "auth::test_user_registration", |_| {
        epic("User Management");
    ...
    run_sample_test(&writer, "test_with_attachments", "api::test_with_attachments", |writer| {
        suite("API Tests");

        step("Make API request", || {});

        // Create text attachment
        let attachment = writer.write_text_attachment("Response Body", r#"{"status": "ok"}"#).unwrap();
    ...
    run_sample_test(&writer, "test_parameterized", "data::test_parameterized", |_| {
    ...
    run_sample_test(&writer, "test_nested_steps", "complex::test_nested_steps", |_| {
        step("Outer step 1", || {
            step("Inner step 1.1", || {});
            step("Inner step 1.2", || {});
        });
    ...
    // 7. Test with links
    run_sample_test(&writer, "test_with_links", "issues::test_with_links", |_| {
        issue("JIRA-123", Some("Login bug".to_string()));
        tms("TMS-456", Some("Login test case".to_string()));
        link("https://example.com/docs", Some("Documentation".to_string()));
    ...
    println!("Run: allure generate {} -o allure-report && allure open allure-report", results_dir);
}
